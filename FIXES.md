...
**Root cause**: The MSAA render pass resolved the 4x MSAA color attachment directly into the LINEAR host-visible target image. This GPU's resolve-to-LINEAR operation smears non-uniform content by the MSAA sample count (4x). Uniform content (full clear, full-coverage blit) resolves correctly; sub-region/non-uniform content smears. This is a tiling-domain issue: MSAA is OPTIMAL (required â€?4x only supported on OPTIMAL), target is LINEAR (required â€?host-visible for CPU readback), and the resolve crosses the OPTIMALâ†’LINEAR tiling boundary.

**Solution**: Resolve the 4x MSAA to a 1x OPTIMAL intermediate image, then `vkCmdCopyImage` (OPTIMALâ†’LINEAR) to the host-visible target. Changes:
...
- `end_render_pass`: after `vkCmdEndRenderPass`, transitions target (GENERALâ†’TRANSFER_DST), `vkCmdCopyImage(resolveâ†’target)`, transitions target back (TRANSFER_DSTâ†’GENERAL for host read).
...
**Symptom**: Multi-pass MSAA accumulation (tiger 239 draws) unreliable â€?`loadOp=LOAD` across render passes read undefined content.

**Root cause**: The render pass had no subpass dependencies (`dependencyCount=0`). Per Vulkan spec, `loadOp=LOAD` across render-pass instances requires a subpass dependency (EXTERNALâ†’subpass) to make the previous pass's `storeOp=STORE` visible. Without it, the MSAA content is undefined at the next pass's load. Also, `end_render_pass` only barriered the target image, not the MSAA color image.

**Solution**:
- `create_render_pass`: added two subpass dependencies (EXTERNALâ†? and 0â†’EXTERNAL, COLOR_ATTACHMENT_OUTPUT stage, COLOR_ATTACHMENT_WRITEâ†’READ access).
- `end_render_pass`: added a pipeline barrier on the MSAA color image (COLOR_ATTACHMENT_WRITEâ†’READ, COLOR_ATTACHMENT_OPTIMALâ†’COLOR_ATTACHMENT_OPTIMAL).
...
**Root cause**: No subpass self-dependency (0â†?) for color-attachment writeâ†’read within a subpass. Without it, a draw's color write is not guaranteed visible to a later draw's blend-dst read in the same subpass.

**Solution**: Added a self-dependency (srcSubpass=0, dstSubpass=0, COLOR_ATTACHMENT_OUTPUTâ†’COLOR_ATTACHMENT_OUTPUT, COLOR_ATTACHMENT_WRITEâ†’READ, `VK_DEPENDENCY_BY_REGION_BIT`) to `create_render_pass`.
...
**Symptom**: Native-blend blits (NONE/SRC_OVER/etc. on common formats) used the no-MSAA path. Switching to 4x MSAA broke `test_blend_premultiply` â€?the CPU-loaded target content (landscape.raw) was lost (hardware blend read empty MSAA, result = src only, no dst blend).

**Root cause**: Hardware pipeline blend's dst is always the color attachment (the MSAA image in the MSAA path). The MSAA does not mirror the target's content when the target was filled externally (CPU `vg_lite_load_raw`/`memcpy` writes the LINEAR target's memory, not the MSAA). So hardware blend reads empty MSAA â†?no dst blend. This is fundamental: the MSAA image is OPTIMAL+device-local (not CPU-writable), and there is no 1xâ†?x copy API (`vkCmdCopyImage`/`vkCmdBlitImage` require matching sample counts; `vkCmdResolveImage` is 4xâ†?x only).
...
## 5. flush_blits â†?flush_render_pass rename

**Date**: 2026-07-02
**Commit**: [7541e63](https://github.com/doujian/vglite_vulkan_2026/commit/7541e63)

**Symptom**: Function name `vg_lite_vulkan_flush_blits` was misleading â€?it ends the active render pass (a generic flush), but is called from clear/blit/draw/pattern/grad/free/finish (7 call sites), not just blits.
...
**Root cause**: `blit_native.frag` outputs `vec4(0,0,0,0)` (transparent black) for out-of-bounds UVs (pixels outside the source texture's [0,1] UV range, e.g., rotated corners). With NONE blend (`blendEnable=FALSE`), this overwrites the dst (blue clear) with black. The CPU-side expected blit (`util.c:compute_expected_blit_pixel`) had the same bug â€?`return 0` for out-of-bounds.

**Solution**:
- `shaders/blit_native.frag`: out-of-bounds UV â†?`discard` (don't write, preserve dst = blue background).
- `util/util.c`: `compute_expected_blit_pixel` out-of-bounds â†?`return dst_px` (preserve destination, matching the GPU's discard).
...
3. **GPU rounding vs CPU truncation**: GPU converts floatâ†?bit using round-to-nearest, but pack_pixel uses truncation. A difference of 1 in 4-bit space = 17 in 8-bit space.
...
**Symptom**: ARGB8888 and ABGR8888 formats had incorrect Vulkan format mappings. ARGB8888 was mapped to B8G8R8A8, ABGR8888 was mapped to R8G8B8A8. Both were wrong â€?VGLite bit field layouts did not match the Vulkan format's byte order.

**Root cause**: VGLite names formats MSB-first (last letter in highest bits). Official bit fields:
- ARGB8888: `31:24=B, 23:16=G, 15:8=R, 7:0=A` â†?memory `[A,R,G,B]`
- ABGR8888: `31:24=R, 23:16=G, 15:8=B, 7:0=A` â†?memory `[A,B,G,R]`

The old mappings assumed ARGB8888=BGRA8888 and ABGR8888=RGBA8888, which is incorrect.

**Solution**:
- `vg_lite_format.c`: ARGB8888 â†?`VK_FORMAT_R8G8B8A8_UNORM` (with swizzle, since Vulkan has no `A8R8G8B8` format). ABGR8888 â†?`VK_FORMAT_A8B8G8R8_UNORM_PACK32` (direct match).
...
**Root cause**: The RGB565/RGBA4444/BGRA4444 clear color remapping in `vg_lite.c` was a driver-specific workaround for Intel Iris Xe, where `vkCmdClearAttachments` inside an MSAA render pass writes `float32[0]` to the high bits of PACK16 formats regardless of the format's channel order. In B5G6R5, high bits = B, so the workaround swapped Râ†”B (`float32[0]=b, [2]=r`). However, in the no-MSAA render pass, `vkCmdClearAttachments` follows the Vulkan spec correctly: `float32[0]` maps to the first named channel (R for B5G6R5). The MSAA workaround was now wrong â€?it put `b` in the R position, producing red instead of blue.
...
**Root cause**: The draw path (`vg_lite_draw.c`) calls `flush_render_pass()` â†?`set_render_target(target)` which creates a new 4x MSAA render pass with `loadOp=LOAD`. The MSAA color image retains stale content from a previous MSAA RP (or undefined on first use). Since the draw pipeline uses `blendEnable=VK_FALSE` (stencil+cover technique), it doesn't read dst for blending â€?but `end_render_pass` resolves the **entire** MSAA surface back to the target, overwriting clear's result in non-drawn areas with stale MSAA content. Unlike the blit path (which calls `seed_msaa` to copy target content into the MSAA image when creating a new RP), the draw path never called `seed_msaa` â€?it didn't need to before because clear always used the MSAA path.
...
2. After `set_render_target(target)`, if `prev_was_no_msaa` â†?call `vg_lite_vulkan_seed_msaa(target, nearest_sampler)`
3. The seed only fires when transitioning from a no-MSAA RP to an MSAA RP on the same target â€?MSAA reuse and pure MSAAâ†’MSAA transitions are unaffected.
...
**Symptom**: After `vg_lite_clear` (no-MSAA) followed by `vg_lite_finish`, a subsequent `vg_lite_draw` on the same target produces black background instead of the cleared color. Only affects the `test_tiled` pattern (clear â†?finish â†?draw â†?finish), not `test_clear` (clear â†?clear â†?finish, same buffer, same submission).

**Root cause**: The `prev_was_no_msaa` check in `vg_lite_draw.c` reads `g_vk_ctx.current_fb_is_no_msaa` to detect when the previous RP was no-MSAA (requiring seed_msaa before MSAA draw). However, `vg_lite_finish()` calls `end_render_pass()` which resets `current_fb_is_no_msaa = 0`. When draw executes in a new command buffer submission after finish, `prev_was_no_msaa` is always 0 â€?seed_msaa is skipped, and the MSAA RP's `loadOp=LOAD` loads stale/undefined MSAA content. The subsequent `end_render_pass` resolve overwrites the target with this stale content, destroying clear's result.
...
The `prev_was_no_msaa` check still handles same-submission transitions (clear â†?draw without finish). The `msaa_needs_seed` flag handles cross-submission transitions (clear â†?finish â†?draw).
...
**Symptom**: MSAA resolve+copy (resolve_image â†?LINEAR target) executed on every `end_render_pass` call, causing massive redundant overhead. For test_tiger (239 draws, 1 finish), 238 unnecessary resolve+copy operations were performed â€?each draw's `flush_render_pass` triggered a full resolve+copy even though the target wouldn't be read until finish.

**Root cause**: `end_render_pass` unconditionally performed the full resolve pipeline (barrier â†?vkCmdCopyImage â†?barrier) for the MSAA path, regardless of whether any consumer needed the target content immediately.

**Solution**: Added `msaa_dirty` flag to `buffer_internal_t` (+ `width`/`height` fields for resolve extent). `end_render_pass` now defers resolve+copy â€?it only marks `msaa_dirty = 1`. A new `vg_lite_vulkan_resolve_msaa_to_target()` function performs the actual resolve+copy lazily at copy-on-read sites:
1. **Draw path** (vg_lite_draw.c, 3 sites): Resolves dirty target BEFORE `set_render_target` (outside active RP â€?vkCmdCopyImage is illegal inside RP)
...
**Symptom**: `test_scale` failed after enabling OBB blit optimization (`use_aabb_blit=1`). Golden verification: 4 passed, 70 failed. Pixel mismatches concentrated at the edges of scaled blit regions â€?affected pixels rendered approximately 50% darker than expected (e.g., got R=56 vs exp R=76). The test uses BI_LINEAR filtered blits at various scale factors (0.25xâ€?.2x).
...
**Solution**: Expanded the OBB by a 1px margin in all directions after clipping to target bounds, before converting to NDC. This ensures edge pixels are covered. The fragment shader's existing UV clamp/discard logic handles the extra coverage safely â€?out-of-range fragments are simply discarded.
...
**Root Cause**: `tests/CMakeLists.txt` defined the `test_tiger` target (lines 53-54) with only `add_executable` + `add_dependencies(spirv_compilation)`, but **no POST_BUILD command to copy the golden directory** into the build output. All comparable golden-using targets had such a copy step â€?`test_tiled` (lines 75-78) used `add_custom_command(TARGET test_tiled POST_BUILD COMMAND copy_directory tiled/golden -> $<TARGET_FILE_DIR:test_tiled>/golden)`, and `test_vector`/`test_clock`/`test_ui` used `copy_if_different` for their `.raw` + `golden.h`. `test_tiger` was the only golden-dependent target missing the copy, so clean rebuild always left `build/tests/golden/tiger.png` absent and the runtime lookup failed.
...
**Root Cause**:
1. `multi_draw` allocated `buffer.format = VG_LITE_RGB565` (B5G6R5, unsupported as linear color-att on this GPU) â†?`vg_lite_allocate` returned `VG_LITE_NOT_SUPPORT` â†?`CHECK_ERROR` jumped to `ErrorHandler`.
2. `ErrorHandler` called `vg_lite_clear_grad(&gradient)`, but `gradient` was an **uninitialized stack variable** (`memset` at L113 ran after `allocate`, never executed). `clear_grad` dereferences `grad->image.handle` (stack garbage, non-NULL) â†?`vg_lite_free` on wild pointer â†?crash.
3. Normal return path (L135 `return SUCCESS`) did **not** `vg_lite_free(&buffer)` â€?each loop iteration leaked the previous `VkImage`/`VkDeviceMemory`/`VkImageView` (static `buffer` handle overwritten without free).

**Solution**:
- Added RGB565â†’BGR565 format fallback (allocate succeeds, avoids ErrorHandler entirely).
...
**Symptom**: After fixing #17 (multi_draw no longer crashes), Vulkan validation reported `VkPipeline OBJ ERROR` on `vkDestroyDevice` â€?a single pipeline leaked.
...
3. **`vlc_parse_path` switch**: Added cases for all new opcodes:
   - **BREAK (0x0A)** â†?emit CLOSE (disconnects subpath without closing)
   - **HLINE/VLINE (+_REL)** â†?emit LINE (preserves prev_y or prev_x)
   - **SQUAD/SCUBIC (+_REL)** â†?reflect previous control point about current point (SVG smooth-curve rule), then emit as QUADâ†’cubic or cubic respectively. Reflection uses `last_cmd_type`/`last_ctrl_x/y`; if previous command wasn't a curve, control = current point (degenerate).
   - **8 ARC variants** â†?call new `arc_to_cubics()` helper. Direction mapping: SCCWARC/LCCWARC = CCW (sweep=1), SCWARC/LCWARC = CW (sweep=0); SC*=small (large_arc=0), LC*=large (large_arc=1).

4. **`arc_to_cubics()`** (new, ~80 lines): Implements the SVG 1.1 spec endpoint-to-center arc conversion (Appendix F.6.5). Steps: (1) degenerate check (zero radius or zero-length â†?line), (2) compute (x1',y1') in rotated frame, (3) scale up radii if too small (lambda check), (4) compute center (cx',cy') with sign from large_arc XOR sweep, (5) rotate center back, (6) compute theta1 and dtheta from dot products, normalize to [-2Ï€, 2Ï€] range based on sweep direction, (7) split into â‰?0Â° segments, emit cubic bezier per segment using the standard k=(4/3)*tan(Î±/2) tangent approximation. Final segment's endpoint is snapped to exact (x2,y2) to avoid drift.

5. **Helper refactor**: Extracted `emit_cubic()` and `emit_line()` helpers that both add the command AND update `last_cmd_type`/`last_ctrl_x/y` â€?ensuring smooth-curve reflection state is consistent across all emission paths (direct CUBIC, QUADâ†’cubic, SQUAD, SCUBIC, arc segments).

**Verification**:
- `gcc -Wall -Wextra -fsyntax-only` on vlc_parser.c: 0 errors, 0 warnings.
- Full build: 38 test targets, 0 errors.
- `test_stroke`: EXIT=0, all 13 cases ran, 12 PNGs generated. stroke3-5.png (fill+stroke combined) now 2823 bytes with 149 unique byte values (previously empty fill â†?smaller/different output). stroke_size values unchanged (268-1564 bytes) since stroke algorithm was already correct.
...
## #20 â€?VG_LITE_DRAW_ZERO incorrectly treated as no-op
...
if (path->path_type == VG_LITE_DRAW_FILL_PATH 
    || path->path_type == VG_LITE_DRAW_ZERO               // â†?treated as FILL
    || path->path_type == VG_LITE_DRAW_FILL_STROKE_PATH)
```

The name "ZERO" refers to "bit0 (stroke) = 0" â€?i.e., zero stroke contribution â€?NOT "render nothing". Fill still renders.
...
**Symptom**: `test_stroke` PNGs showed only the background color with a few black dots near the top-left corner. The stroke geometry (petal shape spanning [0,250]Ã—[0,150]) was entirely missing.

**Root Cause**: `vg_lite_init_path` in `src/vg_lite.c` L1046 had `if (path == NULL || data == NULL) return VG_LITE_INVALID_ARGUMENT;`. The stroke test (and the official CTS pattern) calls `vg_lite_init_path(..., data=NULL)` first, then allocates `path->path = malloc(...)` afterward. With the NULL-data check, `init_path` returned early with `VG_LITE_INVALID_ARGUMENT` before setting any path fields. Since the path was `memset` to 0 beforehand, `path->format` remained `0` (= `VG_LITE_S8`), not the intended `VG_LITE_FP32`. When `vg_lite_append_path` later wrote coordinates, it interpreted them as 1-byte signed integers (S8 format) instead of 4-byte floats â€?all coordinates collapsed to 0. The stroke algorithm then generated a tiny degenerate cluster at the origin instead of the petal outline.

The official `gpu-vglite/VGLite/vg_lite_path.c` L198 only validates `path != NULL`; it does NOT check `data != NULL` because deferred path allocation (init_path â†?malloc â†?append_path) is a supported usage pattern.
...
1. **Render order reversed**: Our port rendered stroke-first then fill (the fill pass was the last statement, L1029). The official `gpu-vglite/VGLite/vg_lite_path.c` does the opposite at L1044 (fill tessellation loop) then L1065 (stroke tessellation loop) â€?fill-first, stroke-second, so stroke overlays fill.
...
- Computed `has_fill` / `has_stroke` flags from `path_type` bitmask (ZERO/FILL_PATH/FILL_STROKE â†?fill; STROKE_PATH/FILL_STROKE â†?stroke).
...
**Verification**:
- Full build: 38 test targets, 0 errors.
- `test_stroke`: EXIT=0, 13 PNGs. `fill_stroke.png` pixel histogram: blue bg 34588px, red fill 28172px, black stroke 2017px (was 1265px before fix â€?stroke now fully visible over fill). `stroke0_0.png` unchanged (stroke-only, not affected by order).
