...
**Verification**: test_uploadTiled 4/4 on both paths (default compute path and VGLITE_DISABLE_STORAGE_UPLOAD=1 copy path, tiled == linear byte-exact); test_uploadBuffer 3/3; full suite 41 PASS / test_gfx3+test_imgIndex FAIL / test_sft_blit crash �� identical to baseline.
...
**Symptom**: MSAA rendering required a per-buffer 4x MSAA color sidecar image plus a fullscreen-triangle "seed" draw on every dirty target switch. Costs: extra memory (a 4x color image per render target) and extra bandwidth (a full-screen seed draw per new render pass, plus an explicit resolve pass on flush) — suboptimal on tile-based GPUs.
...
4. **Seeding**: `seed_msaa` becomes an RP-external `vkCmdCopyImage` target→resolve_image (no fullscreen draw), only needed after no-MSAA RP writes, CPU-visible writes, or sample-count switches.
5. **Call sites**: all 5 seed sites (vg_lite.c blit; vg_lite_draw.c ×4) restructured to flush active RPs and seed before beginning the MSRTSS RP.
...
## Fix: MSRTSS regression �� "channel-swap" corruption in blit path + lost final blit (2026-09-14)

**Symptom**: With `VGLITE_MSRTSS=1` on lavapipe, `test_blit_switch` failed with all pixels `0xFF40A0C0` instead of `0xFFC0A040` (looked like a pure R/B swap), `test_blit_mixed` dumps showed the previous blit's color (magenta instead of cyan �� looked like a G/B swap), `test_scissor` scissors.png differed on exactly half the image including alpha, and blits were ~20x slower (3.4 vs 0.17 ms). Zero validation-layer VUIDs on either run.

**Root Cause**: Not a channel swap at all �� a **lost final render operation per target switch**. The wrong values are literally the previous blit's color (`0xFF40A0C0` == `colors[7]`; magenta == `src[4]`), coincidentally resembling swaps for those color pairs. Under MSRTSS, a target's latest content lives in its 1x `resolve_image` (the RP color attachment) until `resolve_msaa_to_target` copies it to the LINEAR image. Three gaps let that copy be skipped or come too late:
1. The blit MSRTSS branch in `vg_lite_blit` flushed the active RP unconditionally per blit (making the flushed target `msaa_dirty`) but never resolved it �� the next blit sampling that target as source (A��B copy) read a one-op-stale LINEAR image.
2. `vg_lite_finish`/`vg_lite_flush` resolve only `current_fb_internal`; a target left dirty by an earlier switch was never resolved at finish.
3. `vg_lite_buffer_read_ptr` / `vg_lite_buffer_download(_image)` returned LINEAR memory without resolving `msaa_dirty` buffers �� CPU readback saw stale content (this produced the "swap" in the readback-based tests and the stale PNG dumps).

The 20x slowdown came from the same unconditional per-blit flush: every blit got its own MSRTSS RP instance, each costing a full 4x load-replicate + resolve-store. After restoring RP reuse, timing is unchanged (~3.6 ms/blit) �� the remaining cost is **inherent llvmpipe/lavapipe MSRTSS multisampled rendering**, not our per-blit copies. (H1 format/swizzle and H5 driver-bug hypotheses rejected: resolve/attachment formats are identical everywhere and `vkCmdCopyImage` is byte-exact.)

**scissors.png half-image diff**: pre-existing, present in the original baseline artifacts �� the *legacy* path clips the fullscreen seed draw inside `vg_lite_vulkan_seed_msaa` with the user's enabled scissor, leaving the un-scissored half of the 4x sidecar garbage (transparent). The MSRTSS run is the *correct* image (copy-based seed is scissor-immune; the test itself expects blue on the right edge). Not changed by this fix in either mode.

**Solution** (minimal, `src/vg_lite.c` + `src/vg_lite_draw.c`):
1. `vg_lite_blit` MSRTSS branch: flush + seed only when a seed is actually needed (`prev_was_no_msaa || msaa_needs_seed || prev_internal != t_int`); otherwise keep the RP open for legacy-equivalent RP reuse; after any flush, resolve `prev_internal` if `msaa_dirty` before it can go stale.
2. All 4 MSRTSS seed sites in `vg_lite_draw.c`: after the RP flush, resolve a dirty `prev_internal`.
3. New read-side guard `resolve_dirty_before_cpu_read()` (flush �� begin �� resolve �� submit) called from `vg_lite_buffer_read_ptr`, `vg_lite_buffer_download`, and `vg_lite_buffer_download_image` so any CPU read of a dirty target resolves first.

**Verification** (lavapipe, config 1): `VGLITE_MSRTSS=1`: test_blit_switch exit 0 (both targets gold), test_blit_mixed exit 0 with dumps **byte-identical** to the MSRTSS=0 run, test_scissor exit 0; blit_draw/blit_accum/tiger/clock/ui/clear all exit 0 with no new failures (msaaSwitch exit 1 = known pre-existing). `VGLITE_MSRTSS=0`: all outputs byte-identical to the pre-fix baseline. Remaining A/B PNG diffs are unchanged from pre-fix and are resolve-filter rounding on AA edges: tiger 1973/307200 px (0.6%, hairline edges; 695/758 large-deltas on high-variance neighborhoods, values are blends of adjacent colors), clock 1 px (delta 13), ui 22 px (delta 1). scissors differs because the legacy baseline itself is wrong (see above).

**Known issues / follow-up (2026-09-14 final verification)**: (1) Pre-existing legacy-path bug, discovered here: the fullscreen seed draw in `vg_lite_vulkan_seed_msaa` is clipped by the user's enabled scissor, leaving the un-scissored region of the 4x sidecar stale (transparent) �� visible as the legacy scissors.png half-image corruption. The MSRTSS path is unaffected (copy-based seed is scissor-immune) and produces the correct image. Not yet fixed in the legacy path. (2) Test binaries for non-default MSAA configs 2/5/6 (`build_lin_msaa_noobb`, `build_tiled`, `build_opt_msaa_noobb`, built ~11:30) predate the final 20:21 blit fixes; on those stale binaries `test_blit_switch` fails with `VGLITE_MSRTSS=1` (VUID-06881 et al., stale-content pixels). Config 1 (`build/`, rebuilt 20:22 with final sources) is clean �� rebuild 2/5/6 before re-testing them.

## Fix: MSRTSS-enabled clear deferred unnecessarily through the legacy no-MSAA flush path (2026-09-15)

**Symptom**: A `vg_lite_clear` followed by a draw/blit with `VGLITE_MSRTSS=1` always cost 2 render passes + 1 full-screen copy: RP#1 = no-MSAA pass running `vkCmdClearAttachments` on the target (flush_pending_clear_on_target), then a seed copy (target -> resolve_image), then RP#2 = the MSRTSS pass doing the actual draw. The same overhead applied to partial clears. With MSRTSS off (legacy 4x path) this detour is still required, but under MSRTSS all attachments are 1x, so the detour is pure wasted work.

**Root Cause**: `flush_pending_clear_on_target` (a no-MSAA render pass + `vkCmdClearAttachments` on the target image) was written as a workaround for an llvmpipe bug where `vkCmdClearAttachments` R/B-swaps colors on 4x MSAA B5G6R5_PACK16 attachments (FIXES.md, earlier entry). The draw/blit call sites flushed pending clears through it **unconditionally**, and the MSRTSS path inherited that call verbatim. Under MSRTSS the bug's trigger condition (a 4x attachment) does not exist, so the workaround was dead weight. Additionally, the deferred clear was all-or-nothing: any pending clear forced the target through the no-MSAA RP before the MSRTSS RP could begin.

**Solution**: With `msrtss_enabled`, pending clears are resolved inside the MSRTSS render pass itself; the no-MSAA flush is only taken when MSRTSS is off (legacy path unchanged):

1. **Deferred partial clears (new)**: `vg_lite_clear` under MSRTSS now defers *partial* clears too (previously only fullscreen ones), storing the clamped rect (`pending_clear_x/y/w/h`) and color plus a `pending_clear_is_fullscreen` flag in `buffer_internal_t` (src/vg_lite_vulkan.h). The seed decision uses `resolve_current` (RP still open on this target, or msaa_dirty): seed only when the resolve scratch is not authoritative. Non-MSRTSS builds/runs keep the original immediate no-MSAA clear.
2. **Fullscreen clear -> draw**: consumed as `loadOp=CLEAR` via the cached `clear_render_pass` variant passed to `set_render_target_ex` (0 copies, 1 RP). The pending flags are consumed and `msaa_needs_seed` is cleared, since loadOp=CLEAR makes the explicit seed redundant.
3. **Partial clear -> draw**: after the MSRTSS RP begins (and after the stencil clear), `vkCmdClearAttachments` runs with the stored clearRect inside the pass (1 RP, 0 copies). Same pattern used by the existing stencil cleanup and the no-MSAA partial-clear precedent.
4. **All consumption points updated**: vg_lite_draw.c draw_impl + 3 blit variants, and vg_lite.c vg_lite_blit (MSAA branch defers instead of flushing; the no-MSAA-build merge-clear branch now uses the stored rect because `msrtss_enabled` is a runtime flag that can be 1 even in no-MSAA builds). `flush_pending_clear_on_target` itself now replays the stored rect instead of assuming fullscreen.

Net effect for MSRTSS clear->draw: 1 render pass, 0 copies (was 2 RP + 1 copy). Legacy 4x path: byte-for-byte unchanged behavior.

**Verification**: (1) All 8 build configs (LINEAR/OPTIMAL x MSAA on/off x OBB on/off) rebuilt and re-run on the local GPU (MSRTSS not available there -> fallback path, where the new code is fully inert): 46/46 test exes exit 0 per config, except the allowed pre-existing test_sft_blit crash where it occurs. (2) Active-path A/B on lavapipe (Mesa 26.2.0 release-msvc, portable download, config 1 build): `VGLITE_MSRTSS=1` ("[msrtss] ... enabled" confirmed in logs) — 46/46 exit 0, including the clear-heavy tests (test_clear, test_clear_unit, test_clear_dl) and blit-after-clear paths that exercise the new loadOp=CLEAR and in-pass clearAttachments code. `VGLITE_MSRTSS=0` — 46/46 exit 0. PNG dump A/B: 1018/1022 byte-identical; the 4 diffs (clock/scissors/tiger_output/ui) are exactly the pre-existing MSRTSS A/B set documented in the previous entry (resolve-filter AA-edge rounding; scissors differs because the legacy baseline itself is wrong). gfx3/msaaSwitch/imgIndex FAILED-lines are identical under MSRTSS=0 and =1 (lavapipe environment noise, not caused by this change; all exit 0).

**Files**: src/vg_lite_vulkan.h, src/vg_lite.c, src/vg_lite_draw.c

## Fix: OPTIMAL (tiled) targets still routed through the MSRTSS resolve-image scratch (2026-09-15)

**Symptom**: With MSRTSS enabled, an OPTIMAL (tiled) render target still rendered into a per-target 1x `resolve_image` scratch attachment, requiring a seed `vkCmdCopyImage` (target -> scratch) after any RP-external write and a resolve copy (scratch -> target) before the target could be sampled or read back. For OPTIMAL targets this is pure redundancy: the target image itself could be the single-sampled MSRTSS attachment, eliminating the scratch, the seed copies, and the resolve copies entirely (draw->draw target switches: 1 RP 0 copies; finish/readback: 0 copies; upload->draw: 0 copies).

**Root Cause**: The MSRTSS port kept the scratch-attachment structure of the legacy design (where the sidecar was load-bearing for the 4x image). Nothing marked the target image as MSRTSS-capable (VUID-VkFramebufferCreateInfo-samples-06881 requires VK_IMAGE_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT at image creation), so the framebuffer could only reference a purpose-built scratch image. Two enablers were missing: (a) the creation flag + a per-format capability probe on the target allocation path, and (b) render-pass layout bookkeeping matching the target's actual layout lifecycle.

**Key insight**: the target image already lives its whole life in VK_IMAGE_LAYOUT_GENERAL (uploads, readbacks, sampling, and no-MSAA render passes all use GENERAL). A direct-mode MSRTSS render pass therefore uses initialLayout=GENERAL (LOAD) / UNDEFINED (CLEAR) and finalLayout=GENERAL: loadOp=LOAD transparently absorbs every RP-external write to the target, and after the RP the target is immediately sampleable/readable -- zero layout transitions, no seed, no resolve.

**Solution** (gated: `msrtss_supported` && target allocated tiled/OPTIMAL && !g_dump_host_optimal && per-format probe ok; LINEAR targets and probe failures keep the existing scratch path unchanged):

1. **src/vg_lite.c `vg_lite_allocate`**: after the storage decision, probe `vkGetPhysicalDeviceImageFormatProperties(image_fmt, OPTIMAL, usage, storage_flags | VK_IMAGE_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT)`; on success set the MSRTSS flag on `img_ci.flags` and record `internal->msrtss_direct`. The probe also covers the R16_UINT / R8G8B8A8_UNORM storage-created variants used for 16bpp/32bpp compute uploads. Mapped HOST_VISIBLE OPTIMAL dumps (g_dump_host_optimal) are excluded (CPU coherency against attachment writes is fragile; they keep the scratch path). runtime `VGLITE_MSRTSS=0` naturally falls back since `set_render_target_ex` checks `msrtss_enabled` too.
2. **src/vg_lite_vulkan.c `create_render_pass_msrtss(format, color_load, direct)`**: new `direct` parameter selects att[0] initialLayout/finalLayout GENERAL (direct) vs COLOR_ATTACHMENT_OPTIMAL (scratch). Public wrappers keep their signatures and pass direct=0 (layouts do not affect RP compatibility, so the ~10 pipeline-factory callers are unaffected). subpass color_ref.layout stays COLOR_ATTACHMENT_OPTIMAL (the RP performs GENERAL->COLOR conversion at begin/end as needed for the subpass itself, finalLayout=GENERAL returns it). For CLEAR variants att[0].initialLayout=UNDEFINED (contents not needed).
3. **src/vg_lite_vulkan.c `set_render_target_ex`**: `direct = msrtss_enabled && internal->msrtss_direct`; framebuffer = [internal->view, msaa_depth_view] (no ensure_resolve_image); `current_resolve_image = NULL` in direct mode (legacy still registers its scratch). RP caches (`render_pass` / `clear_render_pass`) are stamped with `rp_mode` / `clear_rp_mode` (0=legacy, 1=msrtss scratch, 2=msrtss direct) and rebuilt on mode change, preventing a stale cached RP after a runtime VGLITE_MSRTSS toggle.
4. **`seed_msaa` / `resolve_msaa_to_target`**: early-return no-ops when `internal->msrtss_direct` (resolve also clears msaa_dirty). All 5 seed call sites and the draw/blit paths need no changes -- the seed/resolve machinery simply becomes inert. `end_render_pass` already handles the resolve==NULL case with a GENERAL->GENERAL barrier on the target and does not set msaa_dirty. `vg_lite_clear`'s deferred-clear seed decision treats a direct target as always-authoritative (`resolve_current = internal->msrtss_direct || ...`).
