...
/* Flush + invalidate all MSAA-dependent state: current framebuffer binding,
 * every live buffer's cached MSAA attachments/render passes, and all cached
 * graphics pipelines. Shared by the sample-count switch and the MSRTSS
 * toggle; everything rebuilds lazily in the new mode. */
static void invalidate_all_msaa_state(void)
{
...
vg_lite_error_t vg_lite_vulkan_init(void)
{
    ...
    /* enable_feat2 is static (must outlive vkCreateDevice) — reset it each
     * init so a re-init in the same process never inherits a stale pNext
     * chain (e.g. &msrtss_features from a previous run, which would cycle
     * when msrtss_features.pNext = enable_feat2.pNext is linked again). */
    enable_feat2.pNext = NULL;
    memset(&enable_feat2.features, 0, sizeof(enable_feat2.features));
    ...
    if (msrtss_enable) {
         ...
         * or NULL. Link msrtss_features at the tail in both cases. */
    ...
    g_vk_ctx.use_obb_blit = VGLITE_BLIT_OBB;  /* OBB optimization, configurable via vg_lite_config.h */
                                              ...
                                              VkAttachmentLoadOp color_load)
{
    ...
    /* Self-dependency: a draw's color-attachment write must be visible to a
     * later draw's blend-dst read in the same subpass. */
    deps[2].sType = VK_STRUCTURE_TYPE_SUBPASS_DEPENDENCY_2;
    deps[2].srcSubpass = 0;
    deps[2].dstSubpass = 0;
...
uint64_t vg_lite_vulkan_read_timestamp(uint32_t slot)
{
    ...
    if (res != VK_SUCCESS) {
        ...
        g_vk_ctx.timestamp_query_failed = 1;
        return 0;
    }
    return result;
}
...
vg_lite_error_t vg_lite_vulkan_set_render_target_ex(vg_lite_buffer_t *target, const VkClearValue *clear_value)
{
    ...
    rpbi.clearValueCount = fb_att_count;
...
vg_lite_error_t vg_lite_vulkan_end_render_pass(void)
{
    if (g_vk_ctx.current_fb) {
         ...
         * stale value is load-bearing for the MSRTSS seed condition at the
         * draw/blit call sites (skip re-seed on clean same-buffer rebind);
         * vg_lite_free() nulls it when the buffer is destroyed. */
    }
    return VG_LITE_SUCCESS;
}
...
static VkPipeline create_blit_pipeline_internal(VkFormat format, int blend_group, int mode)
{
    if (mode != 0) {
        ...
        if (!g_vk_ctx.native_fs_frag_shader)
            g_vk_ctx.native_fs_frag_shader = load_shader_module(g_vk_ctx.device, "blit_native_fs_frag");
    ...
    VkPipelineShaderStageCreateInfo stages[2] = {
        ...
        {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_FRAGMENT_BIT,
         mode != 0 ? g_vk_ctx.native_fs_frag_shader : g_vk_ctx.frag_shader, "main", NULL},
    };
    ...
    gp_ci.layout = (mode != 0) ? g_vk_ctx.native_pipeline_layout : g_vk_ctx.blit_pipeline_layout;
...
static VkPipeline create_blit_pipeline_native_msaa(VkFormat format, int blend_group)
{
...
VkPipeline vg_lite_vulkan_get_pipeline(VkFormat format, int blend_group)
{
    for (int i = 0; i < g_vk_ctx.pipeline_cache_count; i++) {
        if (g_vk_ctx.pipeline_cache[i].format == format &&
            ...
            g_vk_ctx.pipeline_cache[i].mode == 0)
    ...
    if (pipeline && g_vk_ctx.pipeline_cache_count < MAX_PIPELINE_CACHE) {
        ...
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].mode = 0;
...
VkPipeline vg_lite_vulkan_get_pipeline_no_msaa(VkFormat format, int blend_group)
{
    for (int i = 0; i < g_vk_ctx.pipeline_cache_count; i++) {
        if (g_vk_ctx.pipeline_cache[i].format == format &&
            g_vk_ctx.pipeline_cache[i].blend_group == blend_group &&
            g_vk_ctx.pipeline_cache[i].mode == 1)
            return g_vk_ctx.pipeline_cache[i].pipeline;
    }
    ...
    if (pipeline && g_vk_ctx.pipeline_cache_count < MAX_PIPELINE_CACHE) {
        ...
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].format = format;
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].blend_group = blend_group;
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].mode = 1;
        g_vk_ctx.pipeline_cache_count++;
    }
    return pipeline;
}
...
VkPipeline vg_lite_vulkan_get_pipeline_native_msaa(VkFormat format, int blend_group)
{
    for (int i = 0; i < g_vk_ctx.pipeline_cache_count; i++) {
        if (g_vk_ctx.pipeline_cache[i].format == format &&
            g_vk_ctx.pipeline_cache[i].blend_group == blend_group &&
            g_vk_ctx.pipeline_cache[i].mode == 2)
            return g_vk_ctx.pipeline_cache[i].pipeline;
    }

    VkPipeline pipeline = create_blit_pipeline_native_msaa(format, blend_group);
    if (pipeline && g_vk_ctx.pipeline_cache_count < MAX_PIPELINE_CACHE) {
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].pipeline = pipeline;
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].format = format;
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].blend_group = blend_group;
        g_vk_ctx.pipeline_cache[g_vk_ctx.pipeline_cache_count].mode = 2;
        g_vk_ctx.pipeline_cache_count++;
    }
    return pipeline;
}

/* Create OBB blit pipeline — same as create_blit_pipeline_internal but uses
 * blit_obb_vert shader module instead of the original blit_vert. */
static VkPipeline create_blit_obb_pipeline_internal(VkFormat format, int blend_group, int mode)
{
    if (!g_vk_ctx.blit_obb_vert_shader)
        g_vk_ctx.blit_obb_vert_shader = load_shader_module(g_vk_ctx.device, "blit_obb_vert");
    ...
    VkPipelineShaderStageCreateInfo stages[2] = {
        {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_VERTEX_BIT, g_vk_ctx.blit_obb_vert_shader, "main", NULL},
    ...
    VkPipelineVertexInputStateCreateInfo vi = {VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO};
    VkPipelineInputAssemblyStateCreateInfo ia = {VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO};
    ia.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;  /* OBB: 4 vertices = 1 strip quad */
    VkPipelineRasterizationStateCreateInfo rs = {VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO};
    rs.lineWidth = 1.0f; rs.cullMode = VK_CULL_MODE_NONE; rs.frontFace = VK_FRONT_FACE_CLOCKWISE;
    VkPipelineMultisampleStateCreateInfo ms = {VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO};
    ms.rasterizationSamples = (mode == 1) ? VK_SAMPLE_COUNT_1_BIT : g_msaa_samples;
...
VkPipeline vg_lite_vulkan_get_pipeline_obb_no_msaa(VkFormat format, int blend_group)
{
    for (int i = 0; i < g_vk_ctx.blit_obb_pipeline_cache_count; i++) {
        if (g_vk_ctx.blit_obb_pipeline_cache[i].format == format &&
            g_vk_ctx.blit_obb_pipeline_cache[i].blend_group == blend_group &&
            g_vk_ctx.blit_obb_pipeline_cache[i].mode == 1)
            return g_vk_ctx.blit_obb_pipeline_cache[i].pipeline;
    }

    VkPipeline pipeline = create_blit_obb_pipeline_internal(format, blend_group, 1);
    if (pipeline && g_vk_ctx.blit_obb_pipeline_cache_count < MAX_PIPELINE_CACHE) {
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].pipeline = pipeline;
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].format = format;
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].blend_group = blend_group;
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].mode = 1;
        g_vk_ctx.blit_obb_pipeline_cache_count++;
    }
    return pipeline;
}
...
VkPipeline vg_lite_vulkan_get_pipeline_obb_native_msaa(VkFormat format, int blend_group)
{
    for (int i = 0; i < g_vk_ctx.blit_obb_pipeline_cache_count; i++) {
        if (g_vk_ctx.blit_obb_pipeline_cache[i].format == format &&
            g_vk_ctx.blit_obb_pipeline_cache[i].blend_group == blend_group &&
            g_vk_ctx.blit_obb_pipeline_cache[i].mode == 2)
            return g_vk_ctx.blit_obb_pipeline_cache[i].pipeline;
    }

    VkPipeline pipeline = create_blit_obb_pipeline_internal(format, blend_group, 2);
    if (pipeline && g_vk_ctx.blit_obb_pipeline_cache_count < MAX_PIPELINE_CACHE) {
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].pipeline = pipeline;
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].format = format;
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].blend_group = blend_group;
        g_vk_ctx.blit_obb_pipeline_cache[g_vk_ctx.blit_obb_pipeline_cache_count].mode = 2;
        g_vk_ctx.blit_obb_pipeline_cache_count++;
    }
    return pipeline;
}
...
void vg_lite_vulkan_destroy_pipelines(void)
{
    ...
    if (g_vk_ctx.native_fs_frag_shader) { vkDestroyShaderModule(g_vk_ctx.device, g_vk_ctx.native_fs_frag_shader, NULL); g_vk_ctx.native_fs_frag_shader = VK_NULL_HANDLE; }
    ...
    for (int i = 0; i < g_vk_ctx.blit_obb_pipeline_cache_count; i++) {
        if (g_vk_ctx.blit_obb_pipeline_cache[i].pipeline)
            vkDestroyPipeline(g_vk_ctx.device, g_vk_ctx.blit_obb_pipeline_cache[i].pipeline, NULL);
    }
    g_vk_ctx.blit_obb_pipeline_cache_count = 0;
    if (g_vk_ctx.blit_obb_vert_shader) { vkDestroyShaderModule(g_vk_ctx.device, g_vk_ctx.blit_obb_vert_shader, NULL); g_vk_ctx.blit_obb_vert_shader = VK_NULL_HANDLE; }
    ...
    for (int i = 0; i < g_vk_ctx.radial_pipeline_cache_count; i++) {
        if (g_vk_ctx.radial_pipeline_cache[i].pipeline)
            vkDestroyPipeline(g_vk_ctx.device, g_vk_ctx.radial_pipeline_cache[i].pipeline, NULL);
    }
    g_vk_ctx.radial_pipeline_cache_count = 0;
    if (g_vk_ctx.radial_pipeline_layout) { vkDestroyPipelineLayout(g_vk_ctx.device, g_vk_ctx.radial_pipeline_layout, NULL); g_vk_ctx.radial_pipeline_layout = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_descriptor_layout) { vkDestroyDescriptorSetLayout(g_vk_ctx.device, g_vk_ctx.radial_descriptor_layout, NULL); g_vk_ctx.radial_descriptor_layout = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_vert_shader) { vkDestroyShaderModule(g_vk_ctx.device, g_vk_ctx.radial_vert_shader, NULL); g_vk_ctx.radial_vert_shader = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_frag_shader) { vkDestroyShaderModule(g_vk_ctx.device, g_vk_ctx.radial_frag_shader, NULL); g_vk_ctx.radial_frag_shader = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_stencil_pipeline) { vkDestroyPipeline(g_vk_ctx.device, g_vk_ctx.radial_stencil_pipeline, NULL); g_vk_ctx.radial_stencil_pipeline = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_cover_vbo) { vkDestroyBuffer(g_vk_ctx.device, g_vk_ctx.radial_cover_vbo, NULL); g_vk_ctx.radial_cover_vbo = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_cover_vbo_mem) { vkFreeMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_vbo_mem, NULL); g_vk_ctx.radial_cover_vbo_mem = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_cover_ibo) { vkDestroyBuffer(g_vk_ctx.device, g_vk_ctx.radial_cover_ibo, NULL); g_vk_ctx.radial_cover_ibo = VK_NULL_HANDLE; }
    if (g_vk_ctx.radial_cover_ibo_mem) { vkFreeMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_ibo_mem, NULL); g_vk_ctx.radial_cover_ibo_mem = VK_NULL_HANDLE; }
...
{
    if (g_vk_ctx.radial_stencil_pipeline) return;

    g_vk_ctx.radial_vert_shader = load_shader_module(g_vk_ctx.device, "radial_vert");
    g_vk_ctx.radial_frag_shader = load_shader_module(g_vk_ctx.device, "radial_frag");

    /* Push constant: mat3 path_matrix (48B) + mat3 radial_coef (48B) +
     * spread_mode (4B) + paint_color (4B) + 4 ints (16B) + blend_mode (4B) = 124B.
     * Layout is identical in size to the pattern pipeline so the same
     * push-constant recording code path can be shared if needed. */
    VkPushConstantRange pc_range = {0};
    pc_range.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    pc_range.offset = 0;
    pc_range.size = 124;

    /* Binding 0: radial 1D ramp LUT (combined image sampler, height == 1). */
    VkDescriptorSetLayoutBinding ds_binding = {0};
    ds_binding.binding = 0;
    ds_binding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    ds_binding.descriptorCount = 1;
    ...
    ds_ci.bindingCount = 1;
    ds_ci.pBindings = &ds_binding;
    if (!g_vk_ctx.radial_descriptor_layout)
        VK_CHECK(vkCreateDescriptorSetLayout(g_vk_ctx.device, &ds_ci, NULL, &g_vk_ctx.radial_descriptor_layout));

    if (!g_vk_ctx.radial_pipeline_layout) {
        VkPipelineLayoutCreateInfo pl_ci = {VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
        pl_ci.setLayoutCount = 1;
        pl_ci.pSetLayouts = &g_vk_ctx.radial_descriptor_layout;
        pl_ci.pushConstantRangeCount = 1;
        pl_ci.pPushConstantRanges = &pc_range;
        VK_CHECK(vkCreatePipelineLayout(g_vk_ctx.device, &pl_ci, NULL, &g_vk_ctx.radial_pipeline_layout));
    }

    VkPipelineShaderStageCreateInfo stages[2] = {
        {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_VERTEX_BIT,   g_vk_ctx.radial_vert_shader, "main", NULL},
        {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_FRAGMENT_BIT, g_vk_ctx.radial_frag_shader, "main", NULL},
    };
    ...
    gp_ci.pDepthStencilState = &stencil_ds;
    gp_ci.pDynamicState = &dyn_ci;
    gp_ci.layout = g_vk_ctx.radial_pipeline_layout;
    gp_ci.renderPass = rp;
    gp_ci.subpass = 0;

    VK_CHECK(vkCreateGraphicsPipelines(g_vk_ctx.device, VK_NULL_HANDLE, 1, &gp_ci, NULL, &g_vk_ctx.radial_stencil_pipeline));

    vkDestroyRenderPass(g_vk_ctx.device, rp, NULL);

    /* Cover quad VBO: fullscreen triangle-list quad (4 verts). */
    VkBufferCreateInfo vbo_ci = {VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    vbo_ci.size = 256;
    vbo_ci.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    VK_CHECK(vkCreateBuffer(g_vk_ctx.device, &vbo_ci, NULL, &g_vk_ctx.radial_cover_vbo));
    VkMemoryRequirements vbo_req;
    vkGetBufferMemoryRequirements(g_vk_ctx.device, g_vk_ctx.radial_cover_vbo, &vbo_req);
    VkMemoryAllocateInfo vbo_alloc = {VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
    vbo_alloc.allocationSize = vbo_req.size;
    vbo_alloc.memoryTypeIndex = (uint32_t)find_memory_type(vbo_req.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    VK_CHECK(vkAllocateMemory(g_vk_ctx.device, &vbo_alloc, NULL, &g_vk_ctx.radial_cover_vbo_mem));
    VK_CHECK(vkBindBufferMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_vbo, g_vk_ctx.radial_cover_vbo_mem, 0));

    void *vbo_ptr;
    VK_CHECK(vkMapMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_vbo_mem, 0, 256, 0, &vbo_ptr));
    float quad[8] = {-1, -1, 1, -1, 1, 1, -1, 1};
    memcpy(vbo_ptr, quad, sizeof(quad));
    vkUnmapMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_vbo_mem);

    /* Cover quad IBO: two triangles. */
    VkBufferCreateInfo ibo_ci = {VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    ibo_ci.size = 6 * sizeof(uint32_t);
    ibo_ci.usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
    VK_CHECK(vkCreateBuffer(g_vk_ctx.device, &ibo_ci, NULL, &g_vk_ctx.radial_cover_ibo));
    VkMemoryRequirements ibo_req;
    vkGetBufferMemoryRequirements(g_vk_ctx.device, g_vk_ctx.radial_cover_ibo, &ibo_req);
    VkMemoryAllocateInfo ibo_alloc = {VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
    ibo_alloc.allocationSize = ibo_req.size;
    ibo_alloc.memoryTypeIndex = (uint32_t)find_memory_type(ibo_req.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    VK_CHECK(vkAllocateMemory(g_vk_ctx.device, &ibo_alloc, NULL, &g_vk_ctx.radial_cover_ibo_mem));
    VK_CHECK(vkBindBufferMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_ibo, g_vk_ctx.radial_cover_ibo_mem, 0));

    void *ibo_ptr;
    VK_CHECK(vkMapMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_ibo_mem, 0, 6 * sizeof(uint32_t), 0, &ibo_ptr));
    uint32_t indices[6] = {0, 1, 2, 0, 2, 3};
    memcpy(ibo_ptr, indices, sizeof(indices));
    vkUnmapMemory(g_vk_ctx.device, g_vk_ctx.radial_cover_ibo_mem);
}

VkPipeline vg_lite_vulkan_get_radial_cover_pipeline(VkFormat format, int blend_group)
{
    for (int i = 0; i < g_vk_ctx.radial_pipeline_cache_count; i++) {
        if (g_vk_ctx.radial_pipeline_cache[i].format == format &&
            g_vk_ctx.radial_pipeline_cache[i].blend_group == blend_group)
            return g_vk_ctx.radial_pipeline_cache[i].pipeline;
    }
    if (!g_vk_ctx.radial_vert_shader) return VK_NULL_HANDLE;
    ...
    if (pipeline && g_vk_ctx.radial_pipeline_cache_count < MAX_PIPELINE_CACHE) {
        g_vk_ctx.radial_pipeline_cache[g_vk_ctx.radial_pipeline_cache_count].pipeline = pipeline;
        g_vk_ctx.radial_pipeline_cache[g_vk_ctx.radial_pipeline_cache_count].format = format;
        g_vk_ctx.radial_pipeline_cache[g_vk_ctx.radial_pipeline_cache_count].blend_group = blend_group;
        g_vk_ctx.radial_pipeline_cache_count++;
    }
    return pipeline;
}
