...
int main(int argc, const char *argv[])
{
    ...
    if (!loaded) loaded = (vg_lite_load_png(&image, "data/gfx2_image.png") == 0);
    if (!loaded) loaded = (vg_lite_load_png(&image, "../tests/data/image.png") == 0);
    if (!loaded) loaded = (vg_lite_load_png(&image, "../tests/data/gfx2_image.png") == 0);
    if (!loaded) {
    ...
    vg_lite_scale((float)buffer.width / 640.0f, (float)buffer.height / 480.0f, &matrix);
